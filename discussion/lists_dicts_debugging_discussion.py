...
def totalprice(cart, prices, discounts):
    totaldiscount = 0
    for index in range(len(cart)):
        if cart[index] in discounts.keys():
            totaldiscount += (1 - discounts[cart[index]]) * prices[index]
        else:
            totaldiscount += prices[index]
    print(totaldiscount)
...
totalprice(cart_items, item_prices, discounts)
